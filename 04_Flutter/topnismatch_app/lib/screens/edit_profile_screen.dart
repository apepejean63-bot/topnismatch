...
import '../widgets/profile_foto_grid.dart';
...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  double _calcularProgreso(Map<String, dynamic> perfil) {
    ...
    if (perfil['ciudad'] != null && perfil['ciudad'].toString().isNotEmpty) campos++;
    if (perfil['intereses'] != null && perfil['intereses'].toString().isNotEmpty) campos++;
  ...
  Future<void> _abrirEditarBio() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => EditarBioScreen(bioActual: _perfil?['bio'] ?? '')),
    );
                  ...
                  onTap: () async {
                    ...
                    try {
                      ...
                      if (titulo == '\u00BFQu\u00E9 busco?') {
                        ...
                        if (mounted) setState(() { _perfil!['objetivo'] = data['objetivo']; });
                      }
                      if (data.isNotEmpty) await _api.editarPerfil(data);
                      await _cargarPerfil();
                    } catch (e) {
  ...
  void _mostrarIdiomasMultiples() {
    final idiomasDisponibles = ['Espa\u00F1ol', 'Ingl\u00E9s', 'Franc\u00E9s', 'Portugu\u00E9s', 'Alem\u00E1n', 'Italiano', 'Chino', 'Otros'];
    final temp = List<String>.from(
      (_perfil?['idioma'] as String? ?? '').split(',').map((e) => e.trim()).where((e) => e.isNotEmpty),
    );
    showModalBottomSheet(
      isScrollControlled: true,
      context: context,
        ...
        builder: (ctx, setModalState) => SafeArea(
          child: Column(
            ...
            children: [
              ...
              Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(2))),
              const SizedBox(height: 12),
              const Text('Idiomas (m\u00E1x. 3)', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              ...idiomasDisponibles.map((idioma) => CheckboxListTile(
                title: Text(idioma),
                value: temp.contains(idioma),
                activeColor: primary,
                onChanged: (val) {
                  setModalState(() {
                    if (val == true && temp.length < 3) temp.add(idioma);
                    else if (val == false) temp.remove(idioma);
                  });
                },
              )),
              Padding(
                ...
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(backgroundColor: primary, minimumSize: const Size(double.infinity, 48)),
  ...
  void _mostrarIntereses() {
    final interesesDisponibles = [
      '\u{1F3B5} M\u00FAsica', '\u{1F3AE} Gaming', '\u26BD F\u00FAtbol',
      '\u2708\uFE0F Viajes', '\u{1F355} Comida', '\u{1F4DA} Lectura',
      '\u{1F3A8} Arte', '\u{1F3CB}\uFE0F Gym', '\u{1F3AC} Cine',
      '\u{1F3A4} Karaoke', '\u{1F6B4} Ciclismo', '\u{1F9D8} Yoga',
    ];
    final temp = List<String>.from(
      (_perfil?['intereses'] as String? ?? '').split(',').map((e) => e.trim()).where((e) => e.isNotEmpty),
    );
    showModalBottomSheet(
      isScrollControlled: true,
      context: context,
                  ...
                  child: Wrap(
                    ...
                    children: interesesDisponibles.map((interes) {
                      ...
                      return GestureDetector(
                        onTap: () {
                          setModalState(() {
                            if (seleccionado) temp.remove(interes);
                            else if (temp.length < 6) temp.add(interes);
                          });
                        },
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
                          decoration: BoxDecoration(
                            color: seleccionado ? primary.withOpacity(0.1) : const Color(0xFFF5F5F5),
                            borderRadius: BorderRadius.circular(20),
                            border: Border.all(color: seleccionado ? primary : Colors.grey.withOpacity(0.3), width: seleccionado ? 1.5 : 1),
                          ),
                          child: Text(interes, style: TextStyle(fontSize: 13, color: seleccionado ? primary : Colors.grey, fontWeight: seleccionado ? FontWeight.bold : FontWeight.normal)),
                        ),
                      );
                    }).toList(),
                  ),
  ...
  Widget build(BuildContext context) {
    ...
    final intereses = (perfil['intereses'] as String?)
                          ...
                          child: Container(
                            ...
                            child: ClipOval(
                              child: perfil['fotoPrincipalUrl'] != null
                                  ? Image.network(perfil['fotoPrincipalUrl'], fit: BoxFit.cover,
                        ...
                        Positioned(
                          ...
                          child: GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: const Icon(Icons.camera_alt, color: primary, size: 14),
                            ),
                          ),
                        ),
                      ],
                      ...
                      child: Column(
                        children: [
                          Row(
                            ...
                            children: [
                              const Text('Completado del perfil', style: TextStyle(color: Colors.white70, fontSize: 12)),
                              Text('${(progreso * 100).toInt()}%', style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
                            ],
                          ),
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                GestureDetector(
                  ...
                  child: _buildCard(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          perfil['bio'] ?? 'Cu\u00E9ntale al mundo qui\u00E9n eres...',
                          style: TextStyle(fontSize: 14, color: perfil['bio'] != null ? Colors.black87 : Colors.grey, height: 1.5),
                        ),
                        const SizedBox(height: 4),
                        Text('${(perfil['bio'] ?? '').length}/300', style: const TextStyle(color: Colors.grey, fontSize: 11)),
                      ],
                    ),
                  ),
                ),
                ...
                _buildCard(
                  ...
                  child: Column(
                    children: [
                      _basicRow(Icons.cake, 'Edad', edad > 0 ? '$edad a\u00F1os' : '-- a\u00F1os', Colors.orange, _abrirDatePicker),
                      _basicRow(Icons.work, 'Profesi\u00F3n', perfil['profesion'] ?? '', Colors.blue,
                          () => _mostrarDialogoEdicion('Profesi\u00F3n', perfil['profesion'] ?? '')),
                      _basicRow(Icons.school, 'Educaci\u00F3n', perfil['educacion'] ?? '', Colors.purple,
                          () => _mostrarSelectorOpciones('Educaci\u00F3n', ['Secundaria', 'T\u00E9cnico', 'Universidad', 'Posgrado', 'Doctorado'])),
                      _basicRow(Icons.language, 'Idioma', perfil['idioma'] ?? '', Colors.teal, _mostrarIdiomasMultiples),
                      _basicRow(Icons.auto_awesome, 'Signo zodiacal', perfil['signoZodiacal'] ?? '', Colors.amber,
                          () => _mostrarSelectorOpciones('Signo zodiacal', ['Aries', 'Tauro', 'G\u00E9minis', 'C\u00E1ncer', 'Leo', 'Virgo', 'Libra', 'Escorpio', 'Sagitario', 'Capricornio', 'Acuario', 'Piscis'])),
                    ],
                  ),
                ),
                const SizedBox(height: 12),
                _buildCard(
                  ...
                  title: '\u00BFQu\u00E9 busco?',
                  child: _basicRow(
                    perfil['objetivo'] == 'RELACION_SERIA' ? Icons.favorite
                        : perfil['objetivo'] == 'AMISTAD' ? Icons.people
                        : perfil['objetivo'] == 'CASUAL' ? Icons.local_fire_department
                        : Icons.help_outline,
                    '\u00BFQu\u00E9 busco?',
                    objetivoTexto,
                    Colors.red,
                    () => _mostrarSelectorOpciones('\u00BFQu\u00E9 busco?', ['Relaci\u00F3n seria', 'Amistad', 'Algo casual', 'No s\u00E9 a\u00FAn']),
                  ),
                ),
                const SizedBox(height: 12),
                _buildCard(
                  ...
                  child: intereses.isEmpty
                      ? Column(
                          ...
                          children: [
                            const Text('Agrega tus intereses', style: TextStyle(color: Colors.grey, fontSize: 14)),
                            const SizedBox(height: 8),
                            GestureDetector(
                              ...
                              child: Container(
                                ...
                                child: const Row(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(width: 6),
                                    Text('Agregar intereses', style: TextStyle(fontSize: 13, color: primary, fontWeight: FontWeight.w600)),
                                  ],
                                ),
                              ),
                            ),
                            ...
                            GestureDetector(
                              ...
                              child: const Row(
                                ...
                                children: [
                                  ...
                                  SizedBox(width: 4),
                                  Text('Editar intereses', style: TextStyle(fontSize: 13, color: primary, fontWeight: FontWeight.w500)),
                                ],
                              ),
                            ),
                          ],
                ...
                _buildCard(
                  ...
                  title: 'Estilo de vida y Redes',
                  child: ProfileLifestyleSection(
                    perfil: perfil,
                    mostrarSelectorOpciones: _mostrarSelectorOpciones,
                  ),
                ),
  ...
  Widget _buildCard({required IconData icon, required String title, required Widget child, Widget? trailing}) {
    return Container(
  ...
  Widget _basicRow(IconData icon, String label, String value, Color iconColor, VoidCallback onTap) {
    return Material(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(value.isEmpty ? 'Agregar' : value,
                        style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: value.isEmpty ? Colors.grey : Colors.black87)),
                  ],
                ),
              ),
